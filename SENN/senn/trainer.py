...
import sys
sys.path.append('/home/sajedhamdan/Desktop/skin_cancer/SENN/senn/models')

from conceptizers import ConvConceptizer
from parameterizers import ConvParameterizer
...
class SENN_Trainer:
    def __init__(self, config):
        ...
        try:
            conceptizer = conceptizers_map[config.conceptizer](**config.__dict__)
            parameterizer = parameterizers_map[config.parameterizer](**config.__dict__)
            aggregator = aggregators_map[config.aggregator](**config.__dict__)
        except KeyError as e:
            print(f"Invalid model key in config: {e}")
        ...
        self.robustness_loss = eval(config.robustness_loss)(
            num_features=147456,                # match J_yx last dimension
            num_concepts=5,
            num_classes=7
        )

        # robustness_loss = cancer_robustness_loss(num_features, num_concepts, num_classes)
